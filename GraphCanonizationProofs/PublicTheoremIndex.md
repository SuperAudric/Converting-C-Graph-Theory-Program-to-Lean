...
| Name | Line | Description | Notes |
|------|------|-------------|-------|
| `Showcase.Iso` | 111-114 | Graph isomorphism on the project’s own `AdjMatrix`: some relabelling of `G` is `H`. Definitionally `CanonSpec.GraphIso`, which is why `①b` needed zero glue. | Definition |
| `Showcase.canonForm?` | 131-149 | **The showcase canonizer, REAL and at the composed force key (2026-07-28)** — encode-free refiner; force = `RecordKey.recordKey`; consume = `RecordCost.recordSupplyFast`. The key swap is a totality gain: `G8` flags at the previous pin and answers here (`Regression` §19). | Definition |
| `Showcase.canonForm?_record` | 151-155 | The record object satisfies the full canonical-form spec — `RecordKey.recordKey_canonizer` through the definitional bridge `canonFormFastS?_eq` (a direct term: everything is defeq). | — |
