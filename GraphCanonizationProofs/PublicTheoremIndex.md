...
| `Showcase.canon_sound` | 474-481 | **`①a` SOUNDNESS, UNCONDITIONAL** — when the canonizer answers, its output is a genuine relabelling of the input. Axiom-clean; the record’s `SoundOpt` half applied directly (`Labelled n` is definitionally the matrix type, so no glue). | — |
| `Showcase.canon_complete` | 483-508 | **`①b` COMPLETENESS, UNCONDITIONAL** — whenever it answers on both inputs, equal forms ⟺ isomorphic. Axiom-clean. **Free**: `CanonSpec.complete_of_isCanonicalFormOpt` says sound ∧ iso-invariant ⟹ complete. ⛔ Do **not** restate the resolver contract as the single unconditional `Covering` — a covering resolver is provably value-invisible, which pins the object to the retired `canonMin` anchor. | — |
| `Showcase.flag_iso_invariant` | 510-516 | **`①c` THE FLAG IS ISO-INVARIANT, UNCONDITIONAL** — flagging is a property of the isomorphism class. Axiom-clean; free from the record’s `IsoInvariantOpt` half, since one equation on `Option`s carries the answer **and** the flag. | — |
| `Showcase.canon_poly_or_flag` | 518-528 | **`②` POLY-OR-FLAG** — proved axiom-clean **on the LEFT disjunct**, so the cost claim needs no flag escape: `cost ≤ 69 * (n+1)^13` on every input. Fan-out `≤ 1` holds by construction and every component is billed, including the deepen guard. ⚠ See `costDeg` for what the degree does and does not certify. | — |
| `Showcase.residue_if_flag` | 556-570 | ★★★ **`③` — DISCHARGED 2026-08-08 (`W-g`), axiom-clean**: if the canonizer flags, the input is provably not a Tinhofer graph. `recordDeepenCell_full_fast.2.2` — the same object `①` and `②` are about, as the standing steer requires. ⚠ The residue is an **over-approximation** (CFI graphs count as residual though their obstruction is linear); narrowing it is W2. | — |
| `Showcase.unhandledResidue_nonvacuous` | 572-588 | ★★ **DISCHARGED 2026-08-04**, axiom-clean, from `RestrictedTransport.tinhoferGraph_nonvacuous`: handled witness `K₁,₂,₃`, residual witness `K₃ ⊔ C₄`. Both are structural facts about the *graphs*; neither mentions the algorithm. | — |
| `Showcase.canonizer` | 595-607 | **THE HEADLINE** — a complete iso-invariant (never wrong) **and** within the explicit polynomial budget, composed from the obligations. Axiom-clean, and since 2026-08-08 the whole file is: zero `sorry`, zero custom axioms, every obligation a property of one object. | — |
...
| `Select.cellBill` | 672-677 | One cell's probe bill, named so the lazy walk and `selProbeCostC` share a single expression. | Definition |
| `Select.selProbeCostC_eq_sum` | 679-681 | `selProbeCostC` **is** the sum of `cellBill` over `nsColours χ`. | — |
| `Select.firesAt` | 683-686 | The per-cell firing test as a `Bool`, so `List.find?` can consume it. | Definition |
| `Select.probeWalk` | 688-704 | ★★ **THE LAZY PROBE** (`W-e`) — walk the cells in increasing colour order, evaluate each supply once **on demand**, bill it, and **stop at the first that narrows to `≤ 1`**, returning that cell together with its narrowing so the committed cell is never re-probed. | Definition |
| `Select.probeWalk_fst` | 706-723 | The walk finds exactly what `List.find?` would, and carries the committed cell's narrowing. | — |
| `Select.probeWalk_snd_le` | 725-736 | The walk bills a **sub-sum** — only the cells it actually probed. | — |
| `Select.find?_sort_eq_min` | 740-773 | ★★ **LEMMA A** — over a **sorted** list of a finset's elements, `List.find?` *is* `Finset.min` of the filter. This is what licenses stopping at the first firing colour. ⚠ Sortedness is essential: `nsColours` is in first-occurrence order, so walking *it* would return the first encountered firing colour, not the least. | — |
| `Select.sort_nonSingletonColours_perm` | 775-781 | The sorted colour list is a permutation of `nsColours χ` — same members, both nodup. What makes the two bill-sums line up. | — |
| `Select.probeWalk_choice` | 783-790 | The walk commits to exactly `selColourC`'s colour. | — |
| `Select.probeWalk_bill_le` | 792-798 | ★★ **LEMMA C** — the walked bill is `≤ selProbeCostC`, so `②` rides the **existing** `selProbeCostC_le`. **No new numerals.** | — |
| `Select.selNodeLazyC` | 802-809 | ★★★ **THE LAZY CELL-INDEXED RESOLVER** — same children as `selNodeC`, strictly smaller bill: only the cells actually probed are evaluated and charged. | Definition |
| `Select.selNodeLazyC_children` | 811-823 | ★★ **THE CHILDREN ARE UNCHANGED** — with `descendS_val_congr`, all that `①` needs. | — |
| `Select.selNodeLazyC_cost_le` | 825-856 | The lazy per-node bill: the probed cells, plus at most one child refinement. | — |
| `Select.descendS_val_congr` | 865-883 | ★★ **LEMMA B** — `descendS`'s **value** depends on the resolver **only through its children**. ⟹ a resolver with the same `.1` and a smaller `.2` inherits the whole `①` capstone, because `NodeTransport`/`NodeTransportAt` are stated purely on `.1`. | — |
| `Select.canonFormS?_congr` | 885-888 | …hence two resolvers with equal children compute the same canonical form. | — |
| `Select.canonFormS?_selNodeLazyC_eq` | 890-896 | The lazy object computes the **same canonical form** as the eager one. | — |
| `Select.selNodeLazyC_canonizer` | 898-904 | ★★★ **`①` FOR THE LAZY RESOLVER** — free from `selNodeC_canonizer`, via lemma B. | — |
| `Select.descentCostS_selNodeLazyC_le` | 906-921 | ★★ **`②` FOR THE LAZY RESOLVER** — the *same* bound as the eager one, reached through `probeWalk_bill_le`. The true cost is smaller; the point is that the proved ceiling does not move. | — |
...
| `Select.cellNarrowC_length_le_one_of_cellOrbitAt` | 1194-1199 | ★★ **ONE CELL BEING A SINGLE ORBIT OF ITS OWN GENERATORS MAKES THAT CELL FIRE** — at **any** cell, with no hypothesis on the key and no reference to `targetColour`. `Consume.rep_eq_of_wordReach` carries no supply hypothesis, so it holds for every `S`. | — |
| `Select.CellResolvedAt` | 1167-1171 | ★★★ **THE DISJUNCTIVE CONDITION** (`W2` stage 1b): the key's **survivors** in cell `c` lie in one orbit of `c`'s own generators — not the whole cell. Reached by consume, by force, or by the two together. | Definition; `CellOrbitAt` is the consume-only special case |
| `Select.cellNarrowC_length_le_one_of_cellResolvedAt` | 1173-1188 | ★★ **THE SURVIVORS BEING ONE ORBIT MAKES THE CELL FIRE.** The original `cellOrbitAt` proof verbatim, with the quantifier weakened to `keepMin` — which is all it ever used. | Holds for every `S` (`Consume.rep_eq_of_wordReach` has no supply hypothesis) |
| `Select.cellResolvedAt_of_cellOrbitAt` | 1189-1192 | **Route 1 — consume**: the whole cell being one orbit is the special case. | via `keepMin_subset` |
| `Select.cellResolvedAt_of_keepMin_le_one` | 1211-1218 | **Route 2 — force, raw form**: at most one survivor ⟹ nothing to certify. **The supply is unconstrained**, so this is the branch that can reach a cell with no symmetry at all. | — |
| `Select.CellSeparatedAt` | 1220-1225 | The force route's hypothesis: the key is **injective on the cell**. Per-cell analogue of `Force.forceBy_singleton_of_separating`'s hypothesis (stated at `branches χ`), and of `KeyComplete.KeySeparatesAt` with the automorphism escape removed. | Definition |
| `Select.keepMin_length_le_one_of_cellSeparatedAt` | 1233-1247 | A separating key leaves at most one survivor — survivors attain the same minimal key value, and injectivity turns that into equality. | `Force.mem_keepMin_iff` + `Descend.lexLeList_antisymm` + `cellList_nodup` |
| `Select.cellResolvedAt_of_cellSeparatedAt` | 1249-1252 | **Route 2 — force**: a key that separates the cell resolves it, **with no supply**. | — |
| `Select.SomeCellOrbit` | 1254-1257 | The socket's hypothesis at one node: SOME non-singleton cell is a single orbit of the generators anchored in it. ⚠ **Consume-only** — unsatisfiable on a rigid node. | Definition |
| `Select.SomeCellResolved` | 1259-1263 | ★★★ **THE DISJUNCTIVE SOCKET'S HYPOTHESIS AT ONE NODE.** | Definition |
| `Select.nodeResolvedC_of_someCellResolved` | 1265-1268 | …which is a `NodeResolvedC` node. | — |
| `Select.someCellResolved_of_someCellOrbit` | 1270-1273 | Nothing regresses: the consume-only hypothesis implies the disjunctive one. | — |
| `Select.someCellResolved_of_cellSeparatedAt` | 1275-1279 | **The force-only instance** — some non-singleton cell is separated by the key. No supply. | — |
| `Select.nodeResolvedC_of_someCellOrbit` | 1281-1283 | …which is a `NodeResolvedC` node. | Now routed through `nodeResolvedC_of_someCellResolved` |
| `Select.handledSC_of_someCellOrbit` | 1285-1296 | ★★★ **THE `W2` SOCKET.** One resolvable cell at every reached non-discrete node ⟹ `HandledSC`, hence never flags. **To widen the handled region, supply a wider hypothesis here; nothing below re-proves.** ⚠ Sufficient, not necessary — a cell can also fire because the *key* separates it. | The `SelectCell` analogue of `TwinFamily.handledS_of_noRigidObstruction` |
| `Select.handledSC_of_someCellResolved` | 1298-1309 | ★★★ **THE DISJUNCTIVE SOCKET** (`W2` stage 1b) — one *resolved* cell at every reached non-discrete node ⟹ `HandledSC`. Strictly weaker than `handledSC_of_someCellOrbit`: satisfiable on a **rigid** node, and the form a force-splits-then-consume-clears argument lands in. **This is the socket a CFI layer theorem must be stated against.** | `handledSC_of_someCellOrbit` is its consume-only specialisation |
| `Select.cellSeparatedAt_of_branchSeparation` | 1326-1336 | Separation on `branches χ` **is** separation on the target cell (`branches_eq_cellList`). | — |
| `Select.someCellResolved_of_branchSeparation` | 1337-1347 | ★★★ **THE FORCE ROUTE AT THE CELL-INDEXED LAYER** (`W2` §9b): a key separating the branch cell resolves the node, **with no condition on the supply** — what lets it reach a cell carrying no symmetry. The rigid stack's firing lemmas end in exactly this disjunct, at `selNode`; this is where they land at the cell-indexed object. | Force-side analogue of `handledSC_of_someCellOrbit` |
...
| `RecordDeepenCell.handledSC_of_resolvedCells` | 205-215 | ★★★ **THE DISJUNCTIVE SOCKET AT THE PUBLISHED OBJECT** (`W2` stage 1b) — `Select.SomeCellResolved` at every reached non-discrete node ⟹ `HandledSC`. The form a **CFI layer** theorem must use: `ResolvableCellAt` is consume-only and unsatisfiable on a rigid node. | — |
| `RecordDeepenCell.not_all_resolved_of_flag` | 217-226 | …and its `③`. Strictly narrower than `not_all_resolvable_of_flag`, because the class satisfying the hypothesis is strictly larger (it includes rigid cells the key separates). | — |
| `RecordDeepenCell.someCellResolved_of_resolvableCellAt` | 227-232 | Nothing regresses: the consume-only obligation implies the disjunctive one. | — |
| `RecordDeepenCell.someCellResolved_of_cellSeparated` | 233-240 | **The force half's entry point** — the key separating *one* non-singleton cell resolves the node, with **no** condition on the supply. | The only route that reaches a cell carrying no symmetry |
