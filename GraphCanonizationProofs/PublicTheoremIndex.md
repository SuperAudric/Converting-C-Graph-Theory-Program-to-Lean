...
| `RigidRefine.SeedsEquivariant` | 1331-1335 | The seed family is iso-invariant: relabelling maps seeds to seeds via the seed action `act σ`. | Definition |
| `RigidRefine.OrderOfEquivariant` | 1337-1342 | The order completion transports (`orderOf (relabel σ)(transport χ)(act σ s) = σ · orderOf adj χ s`). ⚠ Carried per-family — a structural discretizing order = the linear solve on the rigid code (WL won't). | Definition |
| `RigidRefine.seedFrames` | 1344-1348 | The seed-generated frame set — one column order per seed (`(seeds adj χ).image (orderOf adj χ)`). | Definition |
| `RigidRefine.framesEquivariant_seedFrames` | 1350-1366 | **★★ P1 — `FramesEquivariant (seedFrames …)` GENERICALLY** from `SeedsEquivariant` + `OrderOfEquivariant` (pure `Finset.image` algebra). The ① of the poly frame set, independent of size/richness. | — |
| `RigidRefine.card_seedFrames_le` | 1367-1375 | **★ P1 — cardinality reduction** `|seedFrames| ≤ |seeds|` (`Finset.card_image_le`). With a poly seed bound (P2) ⟹ poly frame set ⟹ poly `readAgg` cost. | — |
| `RigidRefine.keyEquivariant_compKey_readAgg_seedFrames` | 1377-1388 | **★★★ P1 capstone** — the mixed-native `①` over a seed-generated frame set, from seed/order equivariance alone. Closes `①` for the poly-frame reader modulo the carried `OrderOfEquivariant` (the recover core). | — |
