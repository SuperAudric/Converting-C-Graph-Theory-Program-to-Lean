...
| `RigidRefine.keyEquivariant_compKey_refineByFrame_adj` | 292-300 | **★★★ Step C:** `compKey`'s `KeyEquivariant` for the concrete `refineByFrame (extractOf rowAdj witChi)` with **ZERO hypotheses** — the rigid-linear `①` machinery instantiated end-to-end; the only remaining rigid-linear content is `②`/`hemit` (faithfulness = carried `ForcingModel.bridge`, per family). | — |
...
| `RigidRefine.ForcedSeparates` | 332-338 | The extraction's forced values separate co-cellular vertices: `χ u = χ v ∧ frameRead u = frameRead v ⟹ u = v`. The per-family faithfulness the `②`/firing needs; carried (on CFI = the interleaving R6, not standalone — the `Z₂^β` gauge coords are `none` = tie = consume's). | Definition |
| `RigidRefine.hemit_of_forcedSeparates` | 340-354 | **★ Step 5 — the `②` reduction.** `Discrete (refineByFrame extract adj χ) ⟸ ForcedSeparates`: `3 * χ v + encOpt (frameRead …)` with `encOpt < 3` splits the χ-digit from the forced-digit (`encOpt_injective`), and `ForcedSeparates` separates within a χ-cell. Family-agnostic. | — |
| `RigidRefine.nodeResolved_compKey_refineByFrame_of_forcedSeparates` | 356-370 | **★★★ `②`/firing capstone on the clean interface:** `NodeResolved` ⟸ `ForcedSeparates` + rigidity (soundness free, `hext`-free). With `keyEquivariant_compKey_refineByFrame` (`①`), the rigid-linear seal for `refineByFrame` rests on exactly `{RefExtractEquivariant` (✅ adj)`, ForcedSeparates}`. | — |
