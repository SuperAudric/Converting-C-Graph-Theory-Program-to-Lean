...
The a-priori cascade-oracle Lean contract (plan: `docs/chain-descent-cascade-oracle-lean-brief.md`). Builds axiom-clean (only `refineStep`/`refineStep_iff` + Lean foundationals), no `sorry`. Phase A = soundness/validity, Phase B = the completeness reduction (wired to the axiom-free orbit-recovery theorems), Phase C = the residual obligations: verdict iso-invariance is *discharged conditionally* (`verdictIsoInvariant_of_complete`), localisation is *sharpened* to its single open implication (`CellsAreOrbits` / `orbitRecoverableAt_iff_cellsAreOrbits`), and the hard half + general-class completeness remain stated-not-assumed. §C.0 also proves the deferred-decisions foundation `real_stays_real`.
