...
| `Select.someCellOrbit_of_targetCellIsOrbit` | 1310-1320 | The target cell is the special case the `Tinhofer` population uses — `branches χ` **is** `cellList χ c` there, so `Consume.CellIsOrbit` at the cell's own supply is literally `CellOrbitAt`. | — |
