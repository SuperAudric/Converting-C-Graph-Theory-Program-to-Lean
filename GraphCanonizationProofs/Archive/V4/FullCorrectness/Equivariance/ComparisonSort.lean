...
`assignRanks cmp₂ (L.map f) = (assignRanks cmp₁ L).map (lift f)` when
`cmp₂ (f a) (f b) = cmp₁ a b` for `a, b ∈ L`. This connects *two different* comparison
functions and is the form needed by Stage B-rel (in `PathEquivarianceRelational.lean`),
where `cmp₁` uses `vts₁`/`br₁` and `cmp₂` uses the σ-permuted `vts₂`/`br₂`.

Proof structure: single-step commutation → foldl-lifted commutation → top-level. The
foldl lemma carries a *pointwise* hypothesis on the elements of the (already-processed)
prefix because the `lastEntry` first component is one of those elements. -/
...
when `cmp₂ (f a) (f b) = cmp₁ a b` for `a, b ∈ L`. -/
