...
| `reachesRigidOrCameron_viaIntraCellBlock` | 4912-4929 | ⊘ **SUPERSEDED** (live: `…_viaBoundedMinMult`). **(Deepest primitive-floor reduction — CONDITIONAL.)** The seal with the crux discharged to the sharper `PersistentTwinGivesIntraCellBlock`. Carries `hClassify` (G3), `hImprim`, and the open `hCrux` (now only nontriviality of the intra-cell block). The converse's algebraic core landed; residue = the isolated nontriviality kernel. Axiom-clean. | — |
...
| `mulUnitHom_apply` | 3371-3373 | F0: `mulUnitHom u x = ↑u * x`. | `@[simp]` |
| `conjHom_apply` | 3383-3385 | F0: `conjHom hd e u = efield hd (e (efield⁻¹ u))`. | `@[simp]` |
| `sigmaCyc_zpow_apply` | 3402-3410 | F0 (load-bearing): `σ^k` acts as multiplication by `α^k` through the field iso (`σ^k u = efield (α^k · efield⁻¹ u)`) — the `σ^k ↦ α^k` reduction both deliverables turn on. | — |
| `exists_npow_fqGen` | 3412-3420 | F0: every nonzero `z ∈ F_q` is a natural power of `α` (the multiplicative-orbit fact, for the irreducibility argument). | — |
| `frobLinear_apply` | 3498-3499 | F1: `frobLinear x = x ^ p`. | `@[simp]` |
| `frobLinear_mul` | 3501-3505 | F1: the **twist relation** `φ(α·x) = α^p · φ(x)` — Frobenius (a ring hom) carries mult-by-`α` to mult-by-`α^p`; the algebraic core of the gap. | — |
| `frobLinear_conj_mulUnit` | 3507-3516 | F1: `φ ∘ (mul α) ∘ φ⁻¹ = (mul α)^p` as linear automorphisms (conjugation carries the generator to its `p`-th power). | — |
| `sigmaPow_zpow_apply` | 3622-3628 | F2b: `σ_β^k` acts as multiplication by `β^k` through the field iso. Generalizes `sigmaCyc_zpow_apply`. | — |
| `exists_zpow_neg_one_iff` | 3824-3835 | §S-stage3-δ In a field, the powers of `-1` are exactly `±1`: `(∃ k:ℤ, (-1)^k = h) ↔ h = 1 ∨ h = -1` (via `Int.even_or_odd` + `Even/Odd.neg_one_zpow`). The `⟨-1⟩={1,-1}` fact in `zpow` form for the `H={±1}` family closure. Axiom-clean. | — |
| `ratio_not_mem_num_out` | 3940-3955 | §S-stage3-δ Subfield ratio fact (numerator out of `K`): `a,b∈K`, `c∉K`, `a≠b` ⟹ `(c−a)/(b−c)∉K`. The round-1 pinning datum (non-`K` point pinned by two `K`-base points). | — |
| `ratio_not_mem_denom_out` | 3957-3974 | §S-stage3-δ Subfield ratio fact (denominator out of `K`): `a,c∈K`, `b∉K`, `c≠a` ⟹ `(c−a)/(b−c)∉K`. The round-2 pinning datum (`K` point pinned by a `K`-base point and a reached non-`K` point). | — |
| `frobLinear_conj_mulUnit'` | 4203-4210 | (separation step 1) `φ ∘ (mul β) ∘ φ⁻¹ = (mul β)^p` for arbitrary unit `β` (generalizes `frobLinear_conj_mulUnit`). | — |
...
| Name | Line | Description | Notes |
|------|------|-------------|-------|
| `Hol.nodup_iterate_relStep` | 159-164 | Every closure round is nodup (the `dedup` tail). | — |
| `Hol.mem_iterate_relStep_mono` | 166-169 | Rounds are monotone on membership. | — |
| `Hol.stab_succ` | 171-184 | A membership-stable round stays stable. | — |
| `Hol.exists_stab` | 186-219 | Some round ≤ `n` is membership-stable (nodup + strict-growth pigeonhole on `toFinset` cards). | — |
| `Hol.finRange_map_perm` | 385-391 | `σ`-image of `finRange` is a permutation of it (nodup + same `toFinset`). | — |
| `Hol.countP_reindex` | 393-398 | `countP` over `finRange` is invariant under precomposition with `σ`. | — |
| `Hol.foldr_min_le` | 488-494 | The fold-min is a lower bound of the list. | — |
| `Hol.foldr_min_mem` | 496-513 | The fold-min of a nonempty `< n`-bounded list is a member. | — |
| `Hol.foldr_min_congr` | 515-522 | Same members ⟹ same fold-min (antisymmetry via the two bounds). | — |
| `Hol.pfT_eq` | 587-597 | The table partner lookup computes `partnerTo` (via `compIdx_eq_iff` + `uniqueFilter_eq_uniqueMem`). | — |
| `Hol.walkOkT_eq` | 599-606 | The table walk validity computes `walkOk`. | — |
| `Hol.tbl_filter_eq` | 608-614 | The table copy-sweep filter equals the spec membership filter. | — |
| `Hol.holMovedT_eq` | 616-624 | The table moved-count computes `holMoved`. | — |
| `Hol.any_walkVals_eq_holHas` | 626-649 | Scanning the materialised walk-value list equals the spec membership test `holHas`. | — |
