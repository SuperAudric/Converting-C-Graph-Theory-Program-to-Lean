...
| `mulUnitHom_apply` | 3022-3024 | F0: `mulUnitHom u x = ↑u * x`. | `@[simp]` |
| `conjHom_apply` | 3034-3036 | F0: `conjHom hd e u = efield hd (e (efield⁻¹ u))`. | `@[simp]` |
| `sigmaCyc_zpow_apply` | 3053-3061 | F0 (load-bearing): `σ^k` acts as multiplication by `α^k` through the field iso (`σ^k u = efield (α^k · efield⁻¹ u)`) — the `σ^k ↦ α^k` reduction both deliverables turn on. | — |
| `exists_npow_fqGen` | 3063-3071 | F0: every nonzero `z ∈ F_q` is a natural power of `α` (the multiplicative-orbit fact, for the irreducibility argument). | — |
| `frobLinear_apply` | 3149-3150 | F1: `frobLinear x = x ^ p`. | `@[simp]` |
| `frobLinear_mul` | 3152-3156 | F1: the **twist relation** `φ(α·x) = α^p · φ(x)` — Frobenius (a ring hom) carries mult-by-`α` to mult-by-`α^p`; the algebraic core of the gap. | — |
| `frobLinear_conj_mulUnit` | 3158-3167 | F1: `φ ∘ (mul α) ∘ φ⁻¹ = (mul α)^p` as linear automorphisms (conjugation carries the generator to its `p`-th power). | — |
| `sigmaPow_zpow_apply` | 3273-3279 | F2b: `σ_β^k` acts as multiplication by `β^k` through the field iso. Generalizes `sigmaCyc_zpow_apply`. | — |
| `exists_zpow_neg_one_iff` | 3475-3486 | §S-stage3-δ In a field, the powers of `-1` are exactly `±1`: `(∃ k:ℤ, (-1)^k = h) ↔ h = 1 ∨ h = -1` (via `Int.even_or_odd` + `Even/Odd.neg_one_zpow`). The `⟨-1⟩={1,-1}` fact in `zpow` form for the `H={±1}` family closure. Axiom-clean. | — |
| `ratio_not_mem_num_out` | 3591-3606 | §S-stage3-δ Subfield ratio fact (numerator out of `K`): `a,b∈K`, `c∉K`, `a≠b` ⟹ `(c−a)/(b−c)∉K`. The round-1 pinning datum (non-`K` point pinned by two `K`-base points). | — |
| `ratio_not_mem_denom_out` | 3608-3625 | §S-stage3-δ Subfield ratio fact (denominator out of `K`): `a,c∈K`, `b∉K`, `c≠a` ⟹ `(c−a)/(b−c)∉K`. The round-2 pinning datum (`K` point pinned by a `K`-base point and a reached non-`K` point). | — |
| `frobLinear_conj_mulUnit'` | 3854-3861 | (separation step 1) `φ ∘ (mul β) ∘ φ⁻¹ = (mul β)^p` for arbitrary unit `β` (generalizes `frobLinear_conj_mulUnit`). | — |
