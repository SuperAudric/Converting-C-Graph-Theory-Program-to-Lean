...
| `affV_card` | 1953-1956 | (Phase 2, M0.3) `card (F_p^d) = p^d` (via `Fintype.card_fun` + `ZMod.card`). | — |
| `instNonemptyAffV` | 2124-2128 | (Phase 2, M1.1 helper) `Nonempty (Fin (p^d))` (`p` prime ⟹ `p^d ≥ 1`). Needed for `orbitalIdx`/diagonal facts used outside the `affineScheme` definition. | Instance |
...
| `reachesRigidOrCameron_viaIntraCellBlock` | 4715-4732 | ⊘ **SUPERSEDED** (live: `…_viaBoundedMinMult`). **(Deepest primitive-floor reduction — CONDITIONAL.)** The seal with the crux discharged to the sharper `PersistentTwinGivesIntraCellBlock`. Carries `hClassify` (G3), `hImprim`, and the open `hCrux` (now only nontriviality of the intra-cell block). The converse's algebraic core landed; residue = the isolated nontriviality kernel. Axiom-clean. | — |
...
| `reachesRigidOrCameron_viaNoConfusionCover` | 1273-1329 | ⊘ **SUPERSEDED** (use `…_viaSmallAutShatters`, the faithful-direction sibling). §S-gate2 (Stage 1b, G-cite) **THE SEAL VIA NO BIG-CONFUSION COVER — the two citations SEPARATED to isolated literals.** The cover dichotomy is factored: the Cameron step **reuses the canonical G3** `hClassify` (via `exhaustiveObstruction_scheme`, no new carry); the only new citation is the **Neumaier direction** `hNeumaier : (∃ over-`B` cover) → IsLarge` (a cover is a near-pencil line system, `card_bigClasses_mul_ge_of_cover`; geometric ⟹ large Aut, monotone under the base). `by_cases` on the cover: cover → `hNeumaier` → `IsLarge`, then primitive → G3 → Cameron / imprimitive → `hImprim` recovers; no cover → `…viaShattering`. Seal `modulo {G3 (hClassify) + Neumaier (hNeumaier) + hcatch + hImprim}` — each a single literal theorem (toward replacing each citation with its proof). Honest scope: `hNeumaier` cited, not proved; row-4 has no line system hence no cover (shatters). Axiom-clean. | — |
| `affinePermFin_apply` | 1975-1978 | (Phase 2, M0.3) `affinePermFin g₀ t i = affineE (g₀ (affineE.symm i) + t)`. | `@[simp]` |
| `affinePermFin_one_mem` | 2028-2031 | (Phase 2, M0.3) A translation (`linear part 1 ∈ G₀`) lies in `affineG`. | — |
| `mulUnitHom_apply` | 2510-2512 | F0: `mulUnitHom u x = ↑u * x`. | `@[simp]` |
| `conjHom_apply` | 2522-2524 | F0: `conjHom hd e u = efield hd (e (efield⁻¹ u))`. | `@[simp]` |
| `sigmaCyc_zpow_apply` | 2541-2549 | F0 (load-bearing): `σ^k` acts as multiplication by `α^k` through the field iso (`σ^k u = efield (α^k · efield⁻¹ u)`) — the `σ^k ↦ α^k` reduction both deliverables turn on. | — |
| `exists_npow_fqGen` | 2551-2559 | F0: every nonzero `z ∈ F_q` is a natural power of `α` (the multiplicative-orbit fact, for the irreducibility argument). | — |
| `frobLinear_apply` | 2637-2638 | F1: `frobLinear x = x ^ p`. | `@[simp]` |
| `frobLinear_mul` | 2640-2644 | F1: the **twist relation** `φ(α·x) = α^p · φ(x)` — Frobenius (a ring hom) carries mult-by-`α` to mult-by-`α^p`; the algebraic core of the gap. | — |
| `frobLinear_conj_mulUnit` | 2646-2655 | F1: `φ ∘ (mul α) ∘ φ⁻¹ = (mul α)^p` as linear automorphisms (conjugation carries the generator to its `p`-th power). | — |
| `sigmaPow_zpow_apply` | 2761-2767 | F2b: `σ_β^k` acts as multiplication by `β^k` through the field iso. Generalizes `sigmaCyc_zpow_apply`. | — |
| `exists_zpow_neg_one_iff` | 2963-2974 | §S-stage3-δ In a field, the powers of `-1` are exactly `±1`: `(∃ k:ℤ, (-1)^k = h) ↔ h = 1 ∨ h = -1` (via `Int.even_or_odd` + `Even/Odd.neg_one_zpow`). The `⟨-1⟩={1,-1}` fact in `zpow` form for the `H={±1}` family closure. Axiom-clean. | — |
| `ratio_not_mem_num_out` | 3079-3094 | §S-stage3-δ Subfield ratio fact (numerator out of `K`): `a,b∈K`, `c∉K`, `a≠b` ⟹ `(c−a)/(b−c)∉K`. The round-1 pinning datum (non-`K` point pinned by two `K`-base points). | — |
| `ratio_not_mem_denom_out` | 3096-3113 | §S-stage3-δ Subfield ratio fact (denominator out of `K`): `a,c∈K`, `b∉K`, `c≠a` ⟹ `(c−a)/(b−c)∉K`. The round-2 pinning datum (`K` point pinned by a `K`-base point and a reached non-`K` point). | — |
| `frobLinear_conj_mulUnit'` | 3342-3349 | (separation step 1) `φ ∘ (mul β) ∘ φ⁻¹ = (mul β)^p` for arbitrary unit `β` (generalizes `frobLinear_conj_mulUnit`). | — |
