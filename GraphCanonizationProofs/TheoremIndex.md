...
| `swapVertexLabels_eq_permute` | `Isomorphic_permute`, `labelEdges_fold_permutes`, `labelEdges_fold_strong`, `permute_of_Isomorphic`, `run_swap_invariant_fwd` | Bridge between concrete `swapVertexLabels` and abstract `permute` action. | — |
...
| `orderInsensitiveListCmp_perm` | `comparePathsBetween_σ_equivariant`, `comparePathsBetween_σ_relational`, `comparePathsBetween_σ_self_eq`, `comparePathsFrom_σ_equivariant`, `comparePathsFrom_σ_relational`, `comparePathsFrom_σ_self_eq` | `orderInsensitiveListCmp cmp L₁ L₂` is invariant under permutations of `L₁` and `L₂`, given a compatible total preorder. | — |
...
| `assignRanksStep` | `assignRanks_aux_density`, `assignRanks_aux_length`, `assignRanks_aux_map_fst`, `assignRanks_aux_rank_le`, `assignRanks_eq_foldl`, `assignRanks_foldl_invariant`, `assignRanks_foldl_lastEntry_fst`, `assignRanks_foldl_map_f`, `assignRanks_foldl_map_f_relational`, `assignRanks_map_of_cmp_respect`, `assignRanks_map_relational`, `assignRanks_rank_eq_at_succ_when_cmp_eq`, `assignRanks_rank_lt_length`, `assignRanks_rank_succ_when_cmp_neq_eq`, `assignRanks_snoc_decompose`, `assignRanks_snoc_decompose_strict`, `assignRanks_strong_invariant`, `assignRanksFoldl_lastEntry_rank_le`, `assignRanksStep_commutes_relational`, `assignRanksStep_commutes_with_f_map`, `assignRanksStep_density_invariant`, `assignRanksStep_fst_length`, `assignRanksStep_fst_map_fst`, `assignRanksStep_preserves_invariant`, `assignRanksStep_rank_le` | Single foldl step for `assignRanks`: appends `(elem, rank)` to accumulator. | private |
...
| `labelEdgesStep` | `labelEdges_fold_permutes`, `labelEdges_fold_strong`, `labelEdges_fold_terminal_aux`, `labelEdges_terminal_rankMap_identity`, `labelEdges_two_graphs_σ_related`, `labelEdges_VtsInvariant_eq_distinct` | The `labelEdgesAccordingToRankings` fold body extracted as a standalone function (swap-and-update). | — |
...
| `PathsFrom_permute_pathsToVertex_perm` | `allBetween_map_f_perm`, `allBetween_two_states_perm`, `between_assignList_σ_rank_closure`, `comparePathsFrom_σ_equivariant`, `comparePathsFrom_σ_relational`, `comparePathsFrom_σ_self_eq`, `mem_allBetween_under_f` | `(p.permute σ).pathsToVertex.Perm (p.pathsToVertex.map (PathsBetween.permute σ))` for length-`vc` `pathsToVertex`. | — |
| `comparePathsBetween_σ_equivariant` | `comparePathsFrom_σ_equivariant`, `comparePathsFrom_σ_self_eq` | `comparePathsBetween vts br (p₁.permute σ) (p₂.permute σ) = comparePathsBetween vts br p₁ p₂` under σ-invariant `vts`/`br` and length conditions. | Single-σ form; relational generalization: `comparePathsBetween_σ_relational` |
| `comparePathsBetween_equivCompat` | `comparePathsFrom_total_preorder`, `comparePathsFrom_σ_equivariant`, `comparePathsFrom_σ_relational`, `comparePathsFrom_σ_self_eq` | Bilateral compat for `comparePathsBetween`: if it returns `.eq`, both arguments compare identically against any third. | — |
| `comparePathsFrom_σ_equivariant` | — | `comparePathsFrom vts br (p₁.permute σ) (p₂.permute σ) = comparePathsFrom vts br p₁ p₂` under σ-invariant `vts`/`br` and length conditions. | Single-σ form; relational generalization: `comparePathsFrom_σ_relational` |
