...
/-! ## Step 9E (P1) — the seed→frame interface: an equivariant, poly-card frame set from an iso-invariant seed family

`readAgg` works over any `FramesEquivariant frames`; `framesUniv` (all `n!`) is correct-but-exponential. The `②`-cost
fix is a POLY equivariant frame set. This step is its structural backbone: a frame set generated by an **iso-invariant
seed family** `seeds` (bounded assignments — the C# affine frames) + an **equivariant order completion** `orderOf`
(one column order per seed). It proves `FramesEquivariant` GENERICALLY and the cardinality reduction `|frames| ≤
|seeds|`, decoupling the three concerns: ① (equivariance, here), cost (`|seeds| ≤ poly`, the concrete seed, P2), and
② (richness = `AggFaithful`, P3).

⚠ The order completion `orderOf` must be a genuine equivariant `Perm` — a fully STRUCTURAL order — which needs
discreteness; for the rigid LINEAR targets that is the linear solve (WL-refinement provably won't discretize the
multipede). So `OrderOfEquivariant` is carried per-family (the recover core / the wall family), NOT general. -/

variable {S : Type*} [DecidableEq S]

/-- The seed family is iso-invariant: relabelling maps seeds to seeds via the seed action `act σ`. -/
def SeedsEquivariant (seeds : AdjMatrix n → Colouring n → Finset S)
    (act : Equiv.Perm (Fin n) → S → S) : Prop :=
  ∀ (σ : Equiv.Perm (Fin n)) (adj : AdjMatrix n) (χ : Colouring n),
    seeds (relabelAdj σ adj) (transportColouring σ χ) = (seeds adj χ).image (act σ)

/-- The order completion is equivariant: the order of the `act σ`-moved seed on the relabelled node = `σ ·` the
original order. (Carried per-family — a structural discretizing order = the linear solve on the rigid code.) -/
def OrderOfEquivariant (orderOf : AdjMatrix n → Colouring n → S → Equiv.Perm (Fin n))
    (act : Equiv.Perm (Fin n) → S → S) : Prop :=
  ∀ (σ : Equiv.Perm (Fin n)) (adj : AdjMatrix n) (χ : Colouring n) (s : S),
    orderOf (relabelAdj σ adj) (transportColouring σ χ) (act σ s) = σ * orderOf adj χ s

/-- **The seed-generated frame set** — one column order per seed. -/
def seedFrames (seeds : AdjMatrix n → Colouring n → Finset S)
    (orderOf : AdjMatrix n → Colouring n → S → Equiv.Perm (Fin n)) :
    AdjMatrix n → Colouring n → Finset (Equiv.Perm (Fin n)) :=
  fun adj χ => (seeds adj χ).image (orderOf adj χ)

/-- **★★ P1 — `FramesEquivariant (seedFrames seeds orderOf)`, GENERICALLY** from `SeedsEquivariant` +
`OrderOfEquivariant`. The seed set transports (`act σ`) and each order transports (`σ ·`), so the image frame set
transports — pure `Finset.image` algebra. This is the ① of the poly frame set, independent of its size/richness. -/
theorem framesEquivariant_seedFrames
    (seeds : AdjMatrix n → Colouring n → Finset S)
    (orderOf : AdjMatrix n → Colouring n → S → Equiv.Perm (Fin n))
    (act : Equiv.Perm (Fin n) → S → S)
    (hs : SeedsEquivariant seeds act) (ho : OrderOfEquivariant orderOf act) :
    FramesEquivariant (seedFrames seeds orderOf) := by
  intro σ adj χ
  simp only [seedFrames]
  rw [hs σ adj χ, Finset.image_image, Finset.image_image]
  refine Finset.image_congr (fun s _ => ?_)
  show orderOf (relabelAdj σ adj) (transportColouring σ χ) (act σ s) = σ * orderOf adj χ s
  exact ho σ adj χ s

omit [DecidableEq S] in
/-- **★ P1 — the cardinality reduction** `|seedFrames| ≤ |seeds|`. Combined with a poly bound on the concrete seed
family (P2, e.g. `|seeds| ≤ n^k` for bounded base rank), this makes the frame set — hence `readAgg`'s per-vertex cost —
polynomial. The frame count never exceeds the seed count (`Finset.card_image_le`). -/
theorem card_seedFrames_le
    (seeds : AdjMatrix n → Colouring n → Finset S)
    (orderOf : AdjMatrix n → Colouring n → S → Equiv.Perm (Fin n))
    (adj : AdjMatrix n) (χ : Colouring n) :
    (seedFrames seeds orderOf adj χ).card ≤ (seeds adj χ).card :=
  Finset.card_image_le

/-- **★★★ P1 capstone — the mixed-native `①` over a SEED-generated frame set.** `compKey (skRead (readAgg
(seedFrames seeds orderOf)))`'s `KeyEquivariant` from the seed/order equivariance alone. With a poly seed bound (P2) the
frame set is poly; this closes the `①` for the poly-frame reader modulo the carried `OrderOfEquivariant` (the recover
core). -/
theorem keyEquivariant_compKey_readAgg_seedFrames
    (seeds : AdjMatrix n → Colouring n → Finset S)
    (orderOf : AdjMatrix n → Colouring n → S → Equiv.Perm (Fin n))
    (act : Equiv.Perm (Fin n) → S → S)
    (hs : SeedsEquivariant seeds act) (ho : OrderOfEquivariant orderOf act) :
    KeyEquivariant (compKey (skRead (readAgg (seedFrames seeds orderOf)))) :=
  keyEquivariant_compKey_readAgg (seedFrames seeds orderOf)
    (framesEquivariant_seedFrames seeds orderOf act hs ho)
