...
| **`Select.lean` / `SelectNode.lean`** | the **resolver-aware selector** (`selNode`): probe every cell against one shared supply evaluation, commit to the least *resolvable* one, `[] =` the **true mutual stall** (`selNode_stall_iff`). **`selNode_children_length_le_one` is UNCONDITIONAL** — there is no branching product in the record object. `NodeResolved`/`HandledS` are the predicates `②`/`③` actually read; **`selNode_canonizer_of_sameOrbits` is KEY-GENERIC**, which is why swapping the record's force key costs one `KeyEquivariant` proof. |
| **`Deepen{Supply,Transport,Crux,Tinhofer,Certified,Located,Key,Exact,Guard}.lean`** | the **C3b consume→force arc**, in that dependency order. `deepenSupply` = the base-symmetry constructor; `Tinhofer` (**renamed from `Amenable`** — AKRV's term; ⛔ do not blanket-rename back, the literature notes quote AKRV's *own* "amenable" for a different class) is its domain hypothesis; `orbKey` is the equivariant force key (`keyEquivariant_orbKey` needs **no** hypothesis) and `orbKeyG S` its **computable** poly-guarded form with the guard **billed** (`certPathCost`, `keyCost_orbKeyG_le`). `guardSupply` = the union guard; §9 = `SameOrbits`-licensing (generic half only — **the instance is R1**, the parked crux). |
| **`KeyComplete.lean`** | **`KeySeparatesAt` / `KeySeparatesAll`** and the **exhaustiveness corollary** `forcedSet_single_orbit_of_keySeparatesAt`: under separation the forced set is one `IsColAut`-orbit, so discarding survivors is sound **without a certificate**. ⚠ `KeySeparatesAt` is F3a's `Hol.KeySeparates` contrapositively — bridge `keySeparatesAt_iff_hol`; the duplication is recorded in **both** files. `keySeparatesAll_rawKey`: separation **alone is cheap**, so the guard buys **equivariance**, not separation. |
| **`TwinFamily.lean`** | **THE CAPABILITY SOCKET AND THE FIRST NAMED FAMILY (W1, 2026-08-04).** `handledS_of_noRigidObstruction`: a **step-closed** class of colourings carrying **no rigid obstruction** (`SchurianAt`, = `¬ RigidObstructionAt` by `schurianAt_iff_no_rigidObstruction`) that holds at the root gives `Select.HandledS` — *widen the class, nothing below re-proves*. `handledS_of_tinhoferGraph` feeds it the **literature Tinhofer class** (`IndivReach` is step-closed **by construction**, so no CAO-propagation obligation arises); `not_tinhoferGraph_of_flagS` is the showcase: **flag ⟹ provably not Tinhofer**. Twin mechanism (`isColAut_swap_of_twin`, `twinCells_step` — the invariant is inherited, so the obligation collapses to the ROOT) + named family (complete multipartite, distinct part sizes) + non-vacuity (`rootCol_eq_of_twin`). §8 crosses **answers → canonized** with a computable equivariant `twinSupply` (`canonized_of_multipartite`). ⚠ `StepClosed` is a HYPOTHESIS not a lemma — `SchurianAt` is not step-preserved in general (that is CAO propagation, refuted at 1-WL). ⚠ `tinhoferGraph_of_root_discrete` is vacuous for the resolvers. |
| **`ForcePick.lean`** | **`forceThenPick`** — the resolver that *cashes* the corollary: force + `take 1`, no supply and no certificate, `①` via `CoveringOfAt` at `N = forcedSet`. **No stall channel** (totality + single-path carry no hypothesis) ⟹ **`forcePick_record`**: *an equivariant, separating, poly force key is a complete polynomial canonizer.* §8's **`readMin`** inhabits the first two clauses unconditionally (exponentially), so **`forcePick_open_clause_is_poly`** pins the whole remaining difficulty to the **poly** clause. |
| **`RecordCost.lean`** | **`②` for the OBJECT OF RECORD** — `supplyCost` bounds for `foldSupplyFast`/`deckSupply`/`deck2Supply`/`kernelSupply`, a `keyCost` bound for `holKeyFast`, the (definitional) `supplyCost_appendSupply`, and the end-to-end `descentCostS_selNode_record_le`. Before this the only end-to-end cost theorems were at `lookaheadKey`+`prunedSupply` — **a different object**. |
| **`RecordKey.lean`** | the **lex-product key combinator** `pairKey` (**plain concatenation** under **`ConstLen k₁`**; ⛔ *not* length-prefixed — that is **shortlex** and re-orders `holKeyFast`'s own narrowing) with `keepMin_pairKey_subset` = **no strength loss**, and the record's composed force key `recordKey = pairKey holKeyFast (orbKeyG guardSupply)` carrying `①` + `②`. |
| `Regression.lean` / `PerformanceTest.lean` | the **build-gating** regression suite (~85 s, in `build.sh` — §17/§17a/§18 gate the executable behaviour of the arc above) and the **off-path** measurements (`lake build ChainDescent.PerformanceTest`, ~10 min, deliberately *not* in `build.sh`). |
