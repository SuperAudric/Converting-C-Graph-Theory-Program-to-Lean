...
* **Firing is graded and measured, never claimed** — and the measured reach is WIDER than the design
  target (2026-07-19, the `wr3` probe; measurements `PerformanceTest` §12):
  - **Coupled-and-chaining** gauges: the second seed resolves one coupled ambiguity and pairwise forcing
    chains the rest (measured: `t3` — fold and deck stall at 3, `deck2Supply` collapses to 1,
    `Regression` §14). This was the design target (C1).
  - **★ INDEPENDENT per-copy gauges (wreath-type, `Z₂ ≀ Z_s`) fire too — the `.getD v` IDENTITY-DEFAULT
    is load-bearing, not junk-tolerance.** A residual ambiguity that is *independent* of the resolved
    part defaults to the identity, and independence is precisely the condition making that completion a
    genuine automorphism, which then verifies (measured: `wr3` = 3 copies matched only on mirror-fixed
    fibers, `Aut ⊇ Z₂³ ⋊ D₃`; fold 6 / deck 3 / `deck2Supply` → 1 at the root, answers end-to-end).
    The earlier "wreath stalls deck2" scope line was WRONG — corrected by measurement.
  - By the same two mechanisms: any `Z₂`-gauge subgroup generated by elements reachable with ≤ 2 seeds —
    in particular every sum-zero (even-subset) code, which is generated by its weight-2 words.
  - **What genuinely stalls it**: a gauge that is the kernel of parity checks of arity ≥ 3 with minimum
    weight ≥ 3 (CFI-style cycle-space gauges) — one assigned wire leaves ≥ 2 candidates at every check
    (no chaining), and no weight-≤2 word exists (no valid identity-default). Odd-degree-base CFI is the
    DEPTH leg's territory (`theorem_1_HOR_cfi_oddDeg` ⟹ `deepMatchSupply` fires); the open witness class
    is the pin-blind (multipede-style, even-degree) SYMMETRIC CFI cover — remaining-work §1C item C3.
