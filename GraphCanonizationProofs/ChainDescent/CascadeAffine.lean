...
**⚠ SUPERSEDED (2026-06-18).** This is the `T := frameBase` instance of `SeparatesAtBase`, and is **FALSE for
minus-type** `VO^-_4(3)` (the symmetric frame is count-blind; see the "Stage B.1c (CORRECTED)" block below). Kept
because it composes (axiom-clean), but the live target is `reachesRigidOrCameron_viaSymmetryBrokenBase` at a
symmetry-broken base. Do not attempt to discharge this predicate. -/
...
`coords_determine` (B.0). **⚠ SUPERSEDED (2026-06-18)** — frame-locked (`Q`-profile recovery at the standard
frame), FALSE for minus-type; the corrected route drops `coords_determine` and goes via `SeparatesAtBase` at a
symmetry-broken base (see the "Stage B.1c (CORRECTED)" block). Confirms the two halves compose: front-half (counts recover the `Q`-profile) +
...
**⚠ SUPERSEDED (2026-06-18).** FALSE for minus-type `VO^-_4(3)`: the one-round count at the symmetric `frameBase`
is blind to the `x₂↔x₃` symmetry (`u=(0,0,1,2)`, `u'=(0,0,2,1)`). The corrected Gauss target is
`IsotropySeparatesAtBase Q T` (count-injectivity, not `Q`-profile recovery) at a symmetry-broken base — see the
"Stage B.1c (CORRECTED)" block. Do not attempt to discharge this predicate. -/
...
/-! #### Stage B.1c (CORRECTED, 2026-06-18) — count-injectivity at a SYMMETRY-BROKEN base (the live target)

**Why this block exists.** A finite probe over `VO^ε_4(3)` (`/tmp/isoprobe*.py`) found the frame-locked predicates
above (`SimilitudeFrameSeparates` / `CountsDetermineFrameQ` / `IsotropyCountsRecoverFrameQ`, all keyed on the
*standard* `frameBase = {0,e₁,…,e_d}`) are **FALSE for minus-type** `VO^-_4(3)`: `Q = x₀x₁+x₂²+x₃²` is symmetric in
`x₂,x₃`, so `u=(0,0,1,2)` and `u'=(0,0,2,1)` have identical one-round counts to the symmetric frame but different
`Q`-profiles. The one-round count is a coarser invariant than the orbit and is *blind* at the symmetric frame. The
scheme still discretizes (iterated WL → 81 singletons in 2 rounds); only the count-at-the-symmetric-frame predicate
is mis-shaped. The FIX: the one-round count **is** injective at a slightly larger, *symmetry-broken* base `T`
(`≈ d+2`, e.g. `frameBase ∪ {p}` for a well-chosen `p`).

**What changes for the proof.** The landed engine `discrete_affineScheme_of_twoRoundDiffSeparates` already accepts an
**arbitrary** `T`; nothing about it was frame-specific. So the corrected target drops the `coords_determine` /
`Q`-profile-recovery architecture entirely (wrong for minus-type — the front half is false) and goes **directly**:
count-injectivity at a bounded base `T` → `Discrete` → `SeparatesAtBoundedBase` → seal. These generic predicates and
capstones replace the frame-locked ones; the latter are kept (axiom-clean, compose) but are SUPERSEDED — their
hypotheses are unprovable as stated for minus-type.

**The Gauss endpoint.** `IsotropySeparatesAtBase Q T` is the pure affine-quadric point-count target the Gauss toolkit
(`ScratchGauss.lean`: the k-fold count via `sum_addChar_multiQuad`) discharges; `separatesAtBase_of_isotropySeparates`
(Witt) lifts it to `SeparatesAtBase`, and `reachesRigidOrCameron_viaIsotropySeparates` composes to the seal. So after
the Gauss build, the *only* remaining open inputs are `OrbitIsIsotropyClass` (Witt, B.1c-i) and a concrete
`IsotropySeparatesAtBase Q T` instance for a symmetry-broken `T` (Gauss, B.1c-ii). -/

/-- **The corrected separation predicate — one-round count-injectivity at an ARBITRARY base `T`.** The
difference-relation count profile at `T` separates all vertices. This is *exactly* the antecedent of the landed
engine `discrete_affineScheme_of_twoRoundDiffSeparates` (with `G₀ := similitudeGroup Q`), now with `T` free rather
than pinned to the symmetric `frameBase`. `SimilitudeFrameSeparates Q` is the (mis-shaped) `T := frameBase` instance.
**The live target:** discharge this for a symmetry-broken `T` of size `≈ d+2`. -/
def SeparatesAtBase (Q : QuadraticForm (ZMod p) (Fin d → ZMod p)) (T : Finset (Fin (p ^ d))) : Prop :=
  ∀ u u' : Fin (p ^ d),
    (∀ (ρ : Fin (p ^ d) → Fin ((affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)).rank + 1))
        (b : Fin ((affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)).rank + 1)),
      (Finset.univ.filter (fun z : Fin (p ^ d) => z ≠ u ∧
        (∀ t ∈ T, (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)).relOfPair (affineE 0)
            (affineE (affineE.symm z - affineE.symm t)) = ρ t)
        ∧ (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)).relOfPair (affineE 0)
            (affineE (affineE.symm z - affineE.symm u)) = b)).card
      = (Finset.univ.filter (fun z : Fin (p ^ d) => z ≠ u' ∧
...
/-- **THE SEAL VIA A SYMMETRY-BROKEN BASE (the corrected node-4 capstone).** For any quadratic form `Q` and any
bounded base `T` (`|T| ≤ bound`) on which the one-round difference-relation counts separate all vertices, the rank-3
SRG `VO^ε` residue discretizes at `T` and seals. Generalizes `reachesRigidOrCameron_viaSimilitudeForm` off the
symmetric `frameBase`. **Carries NO `hSmallAutThin`.** Axiom-clean. -/
theorem reachesRigidOrCameron_viaSymmetryBrokenBase
    {IsCameronScheme : ∀ (m : Nat), SchurianScheme m → Prop} {bound : Nat}
    (Q : QuadraticForm (ZMod p) (Fin d → ZMod p))
    (T : Finset (Fin (p ^ d))) (hcard : T.card ≤ bound) (hsep : SeparatesAtBase Q T) :
    ((SchemeBlockRecovered (p ^ d) (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q))
        ∨ AbelianConsumed (p ^ d) (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)))
        ∨ SchemeRecoveredByDepth (p ^ d)
            (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)) bound)
...
open scoped Classical in
/-- **The Gauss target — isotropy-class count-injectivity at an arbitrary base `T`.** The pure affine-quadric
point-count statement: the isotropy-class count profile at `T` (each `t`: the isotropy class of `z̄ − t̄`) separates
all vertices. NO opaque scheme relations — this is what the Gauss toolkit (`ScratchGauss.lean`) discharges for a
symmetry-broken `T`. Lifted to `SeparatesAtBase` by `separatesAtBase_of_isotropySeparates` (Witt). -/
noncomputable def IsotropySeparatesAtBase (Q : QuadraticForm (ZMod p) (Fin d → ZMod p))
...
/-- **The Witt bridge (arbitrary base) — isotropy-count injectivity ⟹ relation-count injectivity.** Given the Witt
deliverable `OrbitIsIsotropyClass` (relation = injective image of isotropy class), `IsotropySeparatesAtBase Q T`
implies `SeparatesAtBase Q T`: each isotropy filter equals the relation filter at `ρ = relOfIso ∘ σ`, `b = relOfIso c`.
The arbitrary-`T` analogue of `isotropyFrameCountsAgree_of_frameCountsAgree`, in separation form. -/
theorem separatesAtBase_of_isotropySeparates (Q : QuadraticForm (ZMod p) (Fin d → ZMod p))
    {T : Finset (Fin (p ^ d))} (hOrbit : OrbitIsIsotropyClass Q)
  ...
  have key : ∀ w : Fin (p ^ d),
    ...
    · rw [hrel (affineE.symm z - affineE.symm w)]
      exact ⟨fun hh => congrArg relOfIso hh, fun hh => hinj hh⟩
  rw [key u, key u']
  exact hrelcounts (fun t => relOfIso (σ t)) (relOfIso c)

/-- **THE SEAL VIA ISOTROPY COUNTS AT A SYMMETRY-BROKEN BASE (the corrected Gauss-boundary capstone).** End-to-end
with the two heavy pieces at their natural boundary, on the *corrected* (arbitrary-`T`) target: the Witt deliverable
`OrbitIsIsotropyClass` (B.1c-i) plus a concrete `IsotropySeparatesAtBase Q T` for a bounded symmetry-broken `T`
(B.1c-ii, Gauss) compose to the seal. This replaces `reachesRigidOrCameron_viaIsotropyCounts` (which was frame-locked
and is now SUPERSEDED). **Carries NO `hSmallAutThin`.** Axiom-clean. -/
theorem reachesRigidOrCameron_viaIsotropySeparates
    {IsCameronScheme : ∀ (m : Nat), SchurianScheme m → Prop} {bound : Nat}
    (Q : QuadraticForm (ZMod p) (Fin d → ZMod p))
    (T : Finset (Fin (p ^ d))) (hcard : T.card ≤ bound)
    (hOrbit : OrbitIsIsotropyClass Q) (hIso : IsotropySeparatesAtBase Q T) :
    ((SchemeBlockRecovered (p ^ d) (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q))
        ∨ AbelianConsumed (p ^ d) (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)))
        ∨ SchemeRecoveredByDepth (p ^ d)
            (affineScheme (similitudeGroup Q) (neg_mem_similitudeGroup Q)) bound)
  ...
  reachesRigidOrCameron_viaSymmetryBrokenBase Q T hcard
    (separatesAtBase_of_isotropySeparates Q hOrbit hIso)

/-! ##### Witt-FREE route — `OrbitIsIsotropyClass` removed from the seal's critical path

The capstone above carries the **hard** Witt deliverable `OrbitIsIsotropyClass` (the full bijection
relation ↔ isotropy class). But the *separation* bridge only needs the **easy** direction — that the scheme
relation *refines* the isotropy class (`isoClass = g ∘ relOfPair` for some `g`), which is pure
similitude-invariance and is discharged Witt-free below. The hard Witt direction is needed only to *identify*
the scheme as rank-3 `VO^ε`, which the seal (working at any rank) never uses. Net: the seal for the `VO^ε`
residue needs ONLY the Gauss target `IsotropySeparatesAtBase` + a bounded base — no Witt. -/

open scoped Classical in
/-- **The Witt-FREE easy half of `OrbitIsIsotropyClass`.** The scheme relation *refines* the isotropy class:
`isoClass` is a function `g` of `relOfPair (affineE 0) (affineE ·)`. Implied by `OrbitIsIsotropyClass` (the full
bijection), and — unlike it — dischargeable Witt-free (`relationRefinesIsotropy_similitude`). -/
...
/-- The full Witt deliverable implies the easy half (confirming `RelationRefinesIsotropy` is a genuine weakening). -/
...
open scoped Classical in
/-- **The Witt-FREE separation bridge.** `RelationRefinesIsotropy` (the easy half only) + isotropy-count
injectivity ⟹ relation-count injectivity. Proof: fiberwise partition of each isotropy filter by the relation
profile on `T` and at the pivot; the consistency test `g ∘ ρ = σ` is *pivot-independent*, so each fiber's count
agrees between the two pivots via the relation-count hypothesis. The Witt-free analogue of
`separatesAtBase_of_isotropySeparates`. -/
...
isotropy class, purely by similitude-invariance (`affineScheme_relOfPair_eq_iff` + `orbMk_affine_eq_iff` give
the orbit equation; `isoClass_similitude_invariant` finishes). So the capstone's `OrbitIsIsotropyClass` input is
unnecessary. -/
...
`relationRefinesIsotropy_similitude`). The ONLY remaining open input is the Gauss target `IsotropySeparatesAtBase Q T`
(plus the cited `G3`). Supersedes `reachesRigidOrCameron_viaIsotropySeparates` (which carries the redundant Witt
deliverable). **Carries NO `hSmallAutThin`.** Axiom-clean. -/
