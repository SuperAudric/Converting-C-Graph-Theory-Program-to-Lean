...
/-!
# Compare-function σ-equivariance (`FullCorrectness.Equivariance.CompareEquivariant`)

Proves that `comparePathSegments`, `comparePathsBetween`, `comparePathsFrom` are
σ-equivariant under σ-invariant typing/rank functions, and that
`PathsBetween`/`PathsFrom` permute actions produce `List.Perm`-related outputs.

Also exports two small bridge helpers (`betweenRankFn_σ_inv_from_cells`,
`initializePaths_σInv_via_Aut`) used by the Stage B σ-invariance proofs in
`PathEquivariance.lean`.
-/
...
calculatePathRankings_σInvariant_combined  (deep helper, foldl invariant)
  ...
  └── body_preserves_σ_inv     (the deep step; uses Aut G + σ-inv vts)
        ...
        ├── compareFrom σ-equivariant  (via comparePathsFrom_σ_relational ✅)
