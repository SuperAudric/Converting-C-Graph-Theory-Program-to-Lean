...
MODULES=(
  ...
  ChainDescent.Select       # ← Descend (SEL REWRITE increment 1 — the NODE-RESOLVER interface, handoff §6.1 design pass: `descendS` over `N : adj → χ → CostM (List (Fin n × Colouring n))`, `[] = flag` = the true mutual stall; ONE interface covers §6.1 resolver-aware cell selection AND §6.4 hand-forward (each child arrives WITH its refined colouring, licensed by the `NodeProper` equation, never trusted). ★ Safety net `descendS_blind`: the blind instance (least cell + resolver narrowing + per-child refine) equals `descend` as an EXACT CostM equation (value AND cost) ⟹ everything built so far is literally its special case. No exponential: fan-out ≤ 1 by construction, probe additive per node, all-cells harvest ≤ the same tableBound. Next: NodeEquivariant + transport, the fused selNode instance, Reaches/ValidPath widening. Added 2026-07-17)
