...
MODULES=(
  ...
  ChainDescent.PerformanceTest # ← Refine, Consume, Force (REGRESSION GATE, not a library: #guards iso-invariance, distinguishing power, that the descent answers, and that `consume` computes EXACTLY the exhaustive form while pruning it — a regression fails the build)
