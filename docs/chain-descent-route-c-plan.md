...
| **P3** | **Trichotomy, restricted** | large `Aut` ∧ (¬rigid, by P2) ⟹ residue is Cameron ∨ node-4 (both primitive rank-3). | **SKELETON DONE** (2026-07-08, `ScratchConfinementP3.lean`, axiom-clean) — a *carried* reduction, not a clean recomposition. Provable core = wiring the real `exhaustiveObstruction_scheme` (primitive∧rank≥2∧large ⟹ Cameron mod G3). Carried: model bridge `ResidueSchemeModel` (SchurianScheme gap, D0-absorbed), primitivity (`hImprim`), and the **★ threshold-mismatch carrier `hLargeBridge`** — P1's flag gives `residual > n` but the seal's `IsLargeScheme` is super-poly, so `baseMax` must be tuned UP (a cost-model decision) or a threshold-"> n" classification used. `confinement_selectedCellIsOrbit_spine_P3` chains P1∧P2∧P3 through the real seal, carrying only {G3, model, largeBridge, primitivity, Witt}. |
