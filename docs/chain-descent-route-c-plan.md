...
|---|---|---|---|
| **P1** | **Largeness confinement** (the soundness linchpin) | small-`Aut` residue ⟹ orbit-harvest ≤ `w` ⟹ **does not flag**. Contrapositive: flag ⟹ large `Aut`. Pins the budget `w` ≥ the small-`Aut` oracle cost; this is where the cost model (`w`, §4 oracle term) and correctness interlock. Excludes non-Schurian SRGs from Phase-1 flags. | **COST HALF LANDED** (`ScratchCostModelOracle.lean`, core-only, axiom-clean): `oracleCost n b = n^b`, `baseMax n = log₂ n`, `oracleBudget n = n^{log₂ n}`, and **`not_flagsAt_of_base_le`** (base ≤ baseMax ⟹ does not flag) ⟹ contrapositive = the largeness clause, on the cost side. Declared harvest model (D7); a poly `oracleBudget` needs R1. **WIRED INTO THE REAL SPINE** (`ScratchCostModelSpine.spineCappedCanonizerO`, axiom-clean): the flag can now FIRE (`spineCappedCanonizerO_flagsAt_iff`) and `not_flagsAt_of_base_le_spine` is the cost half on the actual descent. **GRAPH SIDE LANDED** (`ScratchConfinementP1.lean`, axiom-clean): `greedy_base_card_le_baseMax` (small residual `|Aut^P ∅| ≤ n` ⟹ a greedy base has length `≤ baseMax n`, from `exists_greedy_base_le_log` + `Nat.log` monotonicity) + `not_flagsAt_of_smallAut_spine` (small residual ⟹ node does NOT flag, on the spine). **RESIDUE SEAM CLOSED** (2026-07-08, `ScratchConfinementP1` + `ScratchConfinement`, axiom-clean): `spineResidualCard k = \|StabilizerAt adj P₀ D_k\|` (level-`k` residual `Aut`), `spineBaseAt k` = greedy base extension length from `D_k` (via `exists_greedy_base_aux`, the pointed form), `spineBaseAt_le_log` proves the greedy bound (was the carried `hgreedy`), and **`flag_imp_large_spine`** = confinement-P1 largeness on the real spine with **NO carried hypotheses**. **P1 DONE end-to-end.** Next: P2/P3/Witt/transport. |
| **P2** | **Deferral confinement** | Phase-1 per-node cost = symmetric-harvest cost only; rigid decisions are *deferred* (cheap push), never expensively harvested. ⟹ a Phase-1 flag is not rigid-caused ("rigid does not entangle into non-rigid"). | **DISCHARGED** (2026-07-08, `ScratchConfinement.flag_imp_symmetric_spine`, axiom-clean). Falls straight out of P1: a flag forces `n < spineResidualCard k` (large residual), but a base has trivial residual (`card=1`, `card_stabilizerAt_eq_one_iff_isBase`), so a flag can't land at a base ⟹ `¬IsBase D_k` (symmetric domain). Rigid/base/IR-core nodes (trivial residual ⟹ `oracleCost=n^0`, cheap) never flag — no separate per-node cost lemma needed. `confinement_selectedCellIsOrbit_spine` = concrete confinement with P1 ∧ P2 wired, only P3/Witt carried. |
| **P3** | **Trichotomy, restricted** | large `Aut` ∧ (¬rigid, by P2) ⟹ residue is Cameron ∨ node-4 (both primitive rank-3). | **SKELETON DONE** (2026-07-08, `ScratchConfinementP3.lean`, axiom-clean) — a *carried* reduction, not a clean recomposition. Provable core = wiring the real `exhaustiveObstruction_scheme` (primitive∧rank≥2∧large ⟹ Cameron mod G3). Carried: model bridge `ResidueSchemeModel` (SchurianScheme gap, D0-absorbed), primitivity (`hImprim`), and the **★ threshold-mismatch carrier `hLargeBridge`** — P1's flag gives `residual > n` but the seal's `IsLargeScheme` is super-poly, so `baseMax` must be tuned UP (a cost-model decision) or a threshold-"> n" classification used. `confinement_selectedCellIsOrbit_spine_P3` chains P1∧P2∧P3 through the real seal. **★ THRESHOLD RESOLVED (2026-07-08)** — it was a SOUNDNESS bug, not just a matching nicety: `baseMax n` raised `log₂n → (log₂n)²` (`ScratchCostModelOracle`), so the flag delivers `2^(baseMax n)=n^{log₂n}<residual` (super-poly, `flag_imp_pow_baseMax_lt`). At `log₂n` a poly-`Aut` non-Schurian SRG (Chang `|Aut|=384>28=n`) would flag ⟹ **unsound** assume-VT-prune; the superlog threshold excludes all poly-`Aut` from flagging (`not_flagsAt_of_residualCard_le_pow`). At the raised threshold `hLargeBridge` is now **provable** (`largeBridge_confinementLargeScheme` via `M.hcard`), so `confinement_selectedCellIsOrbit_spine_P3_discharged` carries only {G3, model, primitivity, Witt}. |
