...
>
> ### ✅✅ 2026-08-17 — **THE READER'S `∏|cell|!` RULER TEST HAS BEEN RUN. IT SPLITS.** (§8 row 11)
> A smaller carrier (`m=10` multipede, `n=60`, ten size-2 **mixed** foot pairs, `|Aut|=1` — the old
> *"smallest is `m=12`"* was a search-grid artefact) plus a **subset**-closure curve brought the test
> inside budget. Leak-free at every row, `∏Sym(cell)`-closed: `s=1` 0/9, `s=2` 0/9, `s=3` 7/9,
> **`s=4` 9/9, `s=5` 9/9**, whole graph discrete from `s≥5`, confirmed to `s=7`. ⟹ **the cells reach
> the orbits.** ⚠ SPLIT is sound under hashing (collisions only merge).
> ★★★ **This closes the gap §6e.4e named** — the base is **rigid**, so it fails by WL-blindness with
> no automorphism to blame, which is exactly the regime the old `P6` measurement could not reach.
> ★★★ **But the mechanism is NOT the ruler.** Null control: rigid **4/4**, cycle **4/4**, path
> **4/4**, **matching 0/4**. What does the work is **cross-pair connectivity — each copy reading
> several cells jointly** — not rigidity, and not injectivity of the reading. That is the `Align`
> picture (a common index set to align against), and it says hypothesis (ii) need not be supplied by
> the ruler *alone*: the composite supplies it.
> ⚠⚠ **Do not over-quote.** `Aut = 1` here, so *"preserving symmetry"* is **vacuous**; what the
...
> * ⚠⚠ **NARROWED 2026-08-16d (§6e.4f #9): a ruler IS a bolt-on device if you bolt it on properly.**
>   With the ruler attached **bijectively** and the family closed under `∏Sym(cell)`, **both** of the
>   Ruler Lemma's hypotheses are measured to hold (`C5`: 120 copies, 12 orbits, exactly 12 colour
>   classes). What kills it as an *algorithm* is the cost `∏|cell|!`, not the mechanism. ⛔ The old claim
>   below holds only for **single-vertex** attachment, where hypothesis (ii) fails:
> * ⛔ **A ruler is NOT a bolt-on device** (§6e.4f #5): attaching
...
3. ★★★ **The general design filter — THE RIGID RULER.** ⚠ Conditional on (A) like everything else
   ...
   unachievable. ⚠ **See §6e.4f #5 and #9**: single-vertex ruler attachment fails, but a **bijective**
   attachment closed under `∏Sym(cell)` satisfies both hypotheses — so the filter is not restricted to
   shared-frame carriers, it is restricted to carriers where paying `∏|cell|!` is acceptable.
...
| 10 | ★★★ **2026-08-16d — do CFI/multipedes give SMALL cells?** (reader) A big graph with many size-2 cells beats a small graph with one giant cell. Multipedes are built to be **asymmetric**, and `Aut = 1` gives two things at once: every non-singleton cell is automatically **mixed**, and individualization breaks no symmetry so it is **free**. `probe_cao_multipede{,2,3}.py`, `probe_cao_multipede_indiv.py`, `probe_cao_multipede_curve.py` | ✅ **YES on the cells.** Partial-Steiner multipedes with full `GF(2)` column rank (⟹ no flip automorphism) are **rigid and 2-WL-non-discrete**, with cells of size **2** (leg pairs) and **4** (gadget vertices) — **every one MIXED**. Smallest: `m=12` legs, `c=12` constraints, **`n = 72`**. ⟹ cost drops from `32! ≈ 10³⁵` to `2¹²·24¹² ≈ 1.5·10²⁰` | ⛔⛔ **BUT BOTH FOLLOW-THROUGHS FAIL.** (a) **Individualization does not help**: rigidity *requires* full rank, so the constraint row space is all of `GF(2)^m` and one individualization propagates everywhere. Measured on 12 instances, `n = 72..272`: the collapse curve is a **cliff**, `10²⁰…10⁷⁴ → 1` in **one** step — no intermediate regime. (b) Restricting the ruler to the **feet only** (legitimate; foot cells are WL-computable) drops it to `2¹² = 4096` copies, but each needs `≥ 24` slots ⟹ `≈ 98,000` vertices ⟹ 2-WL wants `~77 GB`. The window is `n ≲ 1500`, i.e. `m ≤ 6`, and rigid+mixed multipedes do not exist that small |
| 11 | ★★★★ **2026-08-17 — THE READER'S `∏\|cell\|!` TEST HAS NOW BEEN RUN, AND IT SPLITS.** Two corrections got it inside the budget. **(i) A smaller carrier.** Row 10's *"smallest is `m=12`"* was an artefact: `probe_cao_multipede3.py` is the only earlier probe enforcing partial-Steiner and its grid **starts at `m=12`**; the other two sampled triples at random, which at `c ≥ m` is almost never partial-Steiner. ★ The real obstruction is new: full `GF(2)` column rank kills only the **flip** automorphisms, so the constraint hypergraph must **also be asymmetric** — and at small `m` the partial-Steiner maxima are forced to be symmetric designs. Search: `m=7` **0/30** full rank (exhaustive); `m=8` **0/840** asymmetric (exhaustive, `c=8` forced); `m=9` 0 over 120k+ systems per `c`; ★ **`m=10` WORKS** — `n=60`, `c=10`, `\|Aut\|=1`, **ten** size-2 foot cells, all MIXED ⟹ **`2¹⁰ = 1024` copies, 20,480 ruler vertices** (was 4096 / 98,304). **(ii) A curve instead of one row.** ⚠ **PAID-FOR TRAP:** closing over some pairs while holding the rest **fixed** *individualizes* the held ones, and row 10a's cliff then discretizes everything — measured `j = 0..4`, all discrete, useless. ★ The fix is to attach to a **subset** `S` and close over exactly `S`: unattached cells act trivially on the family, so **every row is leak-free**. `probe_cao_multipede_small{,2}.py`, `probe_cao_ruler_curve{,2}.py`, `probe_cao_ruler_threshold.py`, `null_control.py` | ✅✅ **IT SPLITS.** Control (1 copy, not closed) splits ⟹ harness sound. Then, `∏Sym(cell)`-closed and leak-free: `s=1` **0/9**, `s=2` **0/9**, `s=3` **7/9**, `s=4` **9/9**, `s=5` **9/9** (independent rulers × subsets), and at `s ≥ 5` the **whole graph goes discrete** — confirmed to `s=7` (128 copies, `n=1852`). ⟹ **cells reach the orbits.** ⚠ Hash collisions only **merge**, so a reported SPLIT is sound; only `merged` rows could be artefacts (re-checked over 3 seeds). ★★★ **AND THE MECHANISM IS NOT THE RULER.** Null control at `s=4,5`: rigid ruler **4/4**, cycle **4/4**, path **4/4**, **matching 0/4**. A matching links no two pairs, so copy `ε` and `ε⊕e_t` are the *same* attached graph. ⟹ what does the work is **cross-pair connectivity — each copy reading several cells JOINTLY** — not the ruler's rigidity or the injectivity of its reading | ★★★ **This closes the gap §6e.4e named.** That box said (A)'s weakest joint was that the only measurement where the ruler beat the within-copy channel (`P6`) had the copy failing by **automorphism**, not WL-blindness. A rigid multipede has **no automorphism to blame** — it fails purely by WL-blindness — and the leak-free family beats it anyway. ⚠⚠ **What is still NOT tested, and say so when quoting:** `Aut = 1` here, so *"preserving symmetry"* is **vacuous** — what the closure guarantees is that no orbit **information** leaks. ⟹ the **leak** form of the washout objection is refuted by measurement; the **symmetry** form is untouched, and stays out of reach (cheapest mixed cell with non-trivial `Aut` is still rook ⊔ Shrikhande at `∏\|cell\|! = 32!`) |
| 9 | ★★★ **2026-08-16d — the two-closure comparison** (reader's design, mechanism isolated from the mixed cell). Attach the smallest **rigid** graph *bijectively* to a base `G`, close the family under `∏Sym(cell)` (WL-computable, so non-circular) and separately under `Aut(G)`; ask whether the ruler copies' **tag class equals their orbit** = hypothesis (i). `probe_cao_ruler_closure.py` | ✅ **(i) HOLDS EVERYWHERE, at the finest possible value.** `C5` is the sharp row: `∏\|cell\|! = 120` copies, `\|Aut\| = 10` ⟹ **12** orbits, and 2-WL gives **exactly 12** colour classes. Also `P4`, `C4`, `K4`, paw, bull. **(ii) holds too** — the ruler's reading is injective on the whole ruler set. Base cells stayed **= orbits** everywhere, so no automorphism was broken | ⚠⚠ **that the mechanism is NECESSARY.** Every base here has cells = orbits, i.e. 2-WL was already complete — the same weakness §6e.4e logs for (A)'s other measurements. It shows the machinery is *available*, not that it does work nothing else does |
