...
>
> **PICK UP HERE (B1a DONE; increment-4 BACKBONE DONE):** the bridge is closed and the increment-4 anchor-averaging backbone is
> landed (`ScratchIncr4.fail_count_split`/`matching_F_bound`, axiom-clean: `F ≤ c·|V| + |V|·β`). The live frontier is the **two
> analytic inputs** that backbone consumes: **(c)** the per-good-anchor fail bound `c = ¾|V| + O(|V|/√q)` (`c0_le_threequarters` +
> `zeroCount_sq_le` + the `corr` quadric count; the `O(1/√q)` zero-counts DOMINATE the gap, budget the first-moment against them) and
> **(β)** the bad-anchor count `O(|V|/q)` (`#{t₀:¬(hgood∧hnz∧hPu)}`, Schwartz–Zippel **in `t₀`** — distinct from `degenerate_count_le`'s
> `(y,z)`; non-vacuity = distinct radicals for `u≠v`). Then **increment 5** (wire `ScratchMatching.exists_separating_base` with
> `c̄₀ = c/|V|+β/|V| < 1`; supply the frame `S={t,t₀}` + `hne` to `jointIsoCount_ne_of_chiSep_pair` → `zProfileSeparates_of_zSep`).
...
- **`ScratchIncr4.lean`** (NEW 2026-06-26, axiom-clean `[propext, Classical.choice, Quot.sound]`, NOT in build; imports
  `ScratchMatching`) — **increment 4 STRUCTURAL BACKBONE** (the anchor-averaging that produces the matching `F`):
  **`fail_count_split`** (`fail : A→B→Prop`, `A`=probe `t`, `B`=anchor `t₀`; per good anchor `#{a:fail a b}≤c` + `#bad ≤ β`
  ⟹ `#{(a,b):fail} ≤ c·|B| + |A|·β`, pure finite counting via `∑_b #{a:fail a b}` good/bad split) +
  **`matching_F_bound`** (target-indexed form `fail : ι→A→B→Prop` ⟹ `∀ g, #{(t,t₀):fail g} ≤ c·|B|+|A|·β =: F`,
  exactly `exists_separating_base`'s `hF`). So `c̄₀ = F/|V|² = c/|V| + β/|V|`; the two analytic inputs that remain are
  **`c`** (per-good-anchor fail bound `= ¾|V| + O(|V|/√q)`, from `c0_le_threequarters` + the `zeroCount`/`corr` corrections)
  and **`β`** (bad-anchor count `O(|V|/q)`, Schwartz–Zippel in `t₀`). This lemma is where both plug in.
...
This is exactly `exists_separating_base`'s `hF`; the matching closes once `c̄₀ < 1`. **Two analytic inputs remain, and the
backbone shows precisely where each plugs in:**
- **`c` (per-good-anchor fail bound) `= ¾|V| + O(|V|/√q)`.** `fail(t,t₀) ⟹ {χ(I_u)=χ(I_v)} ∨ {I_u(t)=0} ∨ {I_v(t)=0} ∨ {corr≠0}`
  (the bridge capstone `jointIsoCount_ne_of_chiSep_pair` separates only when **both** configs are nondeg, `χ` differ, `corr=0`).
  `#{χ-equal} ≤ ¾|V|` is `c0_le_threequarters`; the zero-counts `#{I_u=0}`, `#{I_v=0}` are `O(|V|/√q)` (`zeroCount_sq_le`);
  `#{corr≠0}` is `O(|V|/q)` (affine quadric). So `c ≤ ¾|V| + O(|V|/√q)` — the `O(1/√q)` zero-counts DOMINATE the gap, not the
  `O(1/q²)` corr (the matching first-moment exponent must be budgeted against `O(1/√q)`).
- **`β` (bad-anchor count) `O(|V|/q)`** = `#{t₀ : ¬(hgood ∧ hnz ∧ hPu)}`, Schwartz–Zippel **in `t₀`** (a different variable than
  `ScratchGoodAnchor.degenerate_count_le`, which is in `(y,z)`); each of the three conditions is the zero-set of a nonzero
  polynomial in `t₀`'s coords (non-vacuity = `∃` good anchor, i.e. `pairForm_u`,`pairForm_v` have distinct radicals for `u≠v`).
