...
- **Step 3e (ii)+(iii) — REMAINING (no more magnitude analysis; counting + Schwartz-Zippel + arithmetic).**
  ...
  pencil discriminant; **the one remaining analytic input, shared with increment 4**) ⟹ `‖T‖ ≤ [(q−1)²q^{d/2} + d(q−1)q^{(d+1)/2}]/q`.
  ...
  (sqrt comparisons, done squared). **All magnitude tools (3b/3c/3d + `normT_le`) are landed; what remains is the good-anchor
  Schwartz-Zippel count + the elementary counting identity + arithmetic.**
