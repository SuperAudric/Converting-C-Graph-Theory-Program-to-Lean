...
- **Step 3e-iii FINISH — REMAINING (the LAST numeric step).** Plug `normT_bucket_bound` (`‖T_ℂ‖ ≤ q√|V| + d|V|/√q`) and
  `zeroCount_sq_le` (`z_u`) into `card_agree_le` ⟹ `c₀ = NS/n ≤ ½ + z_u/n + ‖T_ℂ‖/(2n) ≤ ¾` for `q ≥ q₀` (sqrt comparisons, done
  squared; pick threshold `q₀`). Pure real arithmetic — all combinatorial/analytic inputs landed.
