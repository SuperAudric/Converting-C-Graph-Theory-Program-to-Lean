...
> `c/|V| ≤ 15/16 < 1`. The live frontier is the **one remaining analytic input (β)**: the bad-anchor count `O(|V|/q)` =
> `#{t₀:¬(hgood∧hnz∧hPu∧hPv∧Q(t₀−u)≠0∧Q(t₀−v)≠0)}`, Schwartz–Zippel **in `t₀`** (distinct var from `degenerate_count_le`'s `(y,z)`;
> non-vacuity = distinct radicals for `u≠v`). Then **increment 5** (ℕ-package `c̄₀ = c/|V|+β/|V| < 1` into
> `ScratchMatching.exists_separating_base`; supply the frame `S={t,t₀}` + `hne` to `jointIsoCount_ne_of_chiSep_pair` →
> `zProfileSeparates_of_zSep`).
...
- **`ScratchIncr4.lean`** (NEW 2026-06-26, axiom-clean `[propext, Classical.choice, Quot.sound]`, NOT in build; imports
  ...
  - **Remaining for increment 4 = `β`** (bad-anchor count `O(|V|/q)` = `#{t₀:¬(hgood∧hnz∧hPu∧hPv)}`, Schwartz–Zippel **in
    `t₀`** — distinct var from `degenerate_count_le`'s `(y,z)`; `corr` + `{Q(t₀−u)=0}∪{Q(t₀−v)=0}` fold in here too) +
    the ℕ-packaging into `exists_separating_base` (increment 5).
