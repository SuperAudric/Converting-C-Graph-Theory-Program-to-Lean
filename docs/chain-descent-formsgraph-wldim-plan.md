...
- **Step 3e-iii — REMAINING (the LAST piece; elementary ℤ-counting + arithmetic).** Connect `‖T‖` to `c₀ = NS/n`: the counting
  identity `2·NS ≤ 2·z_u + n + T_ℤ` (`NS = #{t: χ(I_u)=χ(I_v)}`; χ-value case analysis over ℤ) + cast `‖T_ℂ‖ = |T_ℤ|`, then plug
  `normT_bucket_bound` + `zeroCount_sq_le` (`z_u`) ⟹ `c₀ ≤ ¾` for `q ≥ q₀` (sqrt comparisons, done squared). All magnitude/count
  tools are landed; this is pure counting + arithmetic.
