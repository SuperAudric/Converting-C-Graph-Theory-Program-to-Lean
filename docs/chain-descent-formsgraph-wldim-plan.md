...
- **`ScratchPairSep.lean`** (NEW 2026-06-24, compiles axiom-clean, NOT in build) — the **Weil-free per-pair route** core:
  ...
  **unified degenerate-magnitude tool** `norm_sq_sum_addChar_quadForm` (`‖∑ψ(Q)‖²=qᵈ·|radical Q|`, ANY `Q`; powers 3c+3d). Needs
  `import Mathlib.Analysis.Complex.Basic`. See §13 ("DEGENERATE LOCUS FINISHED" + "INCREMENT 3 — PLAN", step 3b + UNIFIED TOOL DONE).
...
- **★ Step 3b — `|M| ≤ q^{d/2}` on the nondeg locus — DONE (2026-06-24, `ScratchPairSep.lean`, axiom-clean).** Three lemmas:
  ...
  **`norm_addChar_eq_one`** (`AddChar` values into `ℂ` are unit-modulus — `(card K)`-th roots of unity via `map_nsmul_eq_pow`
  ...
  have norm 1, the `∏χ(wᵢ)` factor has norm ≤ 1 via `Finset.prod_le_one`). Together: `‖M‖² ≤ (card K)^d = q^d`. **Step 3c —
  `|M| ≤ q^{(d+1)/2}` on the residual conic** (corank-1 degenerate Gauss; `sum_addChar_radical_vanish` already zeroes the
  `L(r)≠0` part) — still TODO.
