...
- **`ScratchPairSep.lean`** (NEW 2026-06-24, compiles axiom-clean, NOT in build) — the **Weil-free per-pair route** core:
  ...
  **unified degenerate-magnitude tool** `norm_sq_sum_addChar_quadForm` (`‖∑ψ(Q)‖²=qᵈ·|radical Q|`, ANY `Q`), its **with-linear
  ...
  + the **zero-count bound (3d)** `zeroCount_sq_le` (`(z·q−qᵈ)²≤(q−1)²·qᵈ·|radical P|`). See §13 ("INCREMENT 3 — PLAN", steps
  3b+3c+3d + UNIFIED TOOL DONE). Open tail = 3e (`|T|` bound, gated on good-anchor #conic count [shared w/ inc4] + the `c₀` counting/arithmetic wrappers).
...
- **Step 3e — assemble `T` and `c₀` (REMAINING).** (i) `|T|` bound: `gaussSum²·T = ∑_{y,z≠0} χ(y)χ(z)·M(y,z)`
  (`pairCharSum_factor_gen`); `|T| ≤ [(q−1)²·q^{d/2} + #conic·q^{(d+1)/2}]/q` using `norm_sq_pairSum_le` per `(y,z)`, where
  `#conic = #{(y,z): F degenerate} ≤ d(q−1)` is the **good-anchor count (the one remaining analytic input — Schwartz-Zippel on
  the pencil discriminant, shared with increment 4)**. (ii) `c₀` counting identity `NS ≤ z_u + ½(n+T)` (χ-value case analysis).
  (iii) arithmetic: plug `zeroCount_sq_le` + `|T|` ⟹ `c₀ ≤ ¾` for `q ≥ q₀` (sqrt comparisons, squared). **All magnitude tools are
  landed; what remains is the good-anchor count + the elementary counting/arithmetic wrappers.**
