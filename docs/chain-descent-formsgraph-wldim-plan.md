...
- **Step 3e (ii)+(iii) — REMAINING (no more magnitude analysis; counting + arithmetic).**
  (a) **good-anchor count** — `#{(y,z): F_{y,z} degenerate} ≤ d·q` is now **FULLY PROVEN** (`degenerate_count_le`, see ★★ above);
  the Schwartz-Zippel input shared with increment 4 is CLOSED. With the corank-uniform deg bound, `‖T‖ ≤ [(q−1)²q^{d/2} +
  d·q·q^{(d+1)/2}]/q`. (Open glue: χ-norm `‖χy‖∈{0,1}`, nondeg/deg sum split, plug `degenerate_count_le` + the uniform bound.)
