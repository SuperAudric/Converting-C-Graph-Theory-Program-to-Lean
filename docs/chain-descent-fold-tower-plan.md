...
> **✅✅ SCOPE CORRECTED BY MEASUREMENT (2026-07-19, same day — the C2 wreath probe, `PerformanceTest`
> §12): the "wreath-type `k ≥ 2` independent decisions still stall" line was WRONG — `wr3` (3 copies
> matched only on mirror-fixed fibers, `Aut ⊇ Z₂³ ⋊ D₃`) FIRES: fold 6 / deck 3 / deck2 → 1, answers
> end-to-end.** The `.getD v` identity-default is load-bearing: an *independent* residual ambiguity
> defaults to identity, and independence is exactly what makes that completion an automorphism (verify
> accepts; on coupled residuals the same default emits junk that verify rejects — sound either way).
> Measured reach = coupled-and-chaining (t3) + independent (wr3) + everything generated by ≤2-seed
> elements (e.g. all sum-zero codes, generated by weight-2 words). **The genuine residual: gauges =
> kernels of arity-≥3 parity checks with min weight ≥ 3 (CFI cycle-space shape) — no chaining, no valid
> default.** Odd-degree-base CFI is the DEPTH leg's (`theorem_1_HOR_cfi_oddDeg`); the open witness class
> = SYMMETRIC pin-blind (multipede-style even-degree) CFI covers → remaining-work §1C **C3** (C2 is
> CLOSED-BY-MEASUREMENT, folded into C3).
>
