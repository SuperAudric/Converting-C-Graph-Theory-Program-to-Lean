...
|---|---|---|---|---|
| **`Suzuki.SuzukiFormsDetermine`** | *(removed)* `ScratchRouteC` §Suzuki | σ-twisted ovoid determiner; was Suzuki 1962 / `Sz(q)` 2-trans | was Route-C Suzuki `separates` | **✅ DISCHARGED 2026-07-04** — proved outright (§3.1). Deleted. |
| **`NondegQuadricDeterminesForm`** | `ScratchRouteC` (F4 `recoveredForm_colouring_equivariant`) | quadric Nullstellensatz: nondeg quadric cone ⟹ form unique up to scalar (`p≠2,d≥4`); Hirschfeld | **F4 only** (not the seal) | **◑ ROUTE FOUND (§3.2)** — dischargeable for `q=p` via vanishing-space transport; full Nullstellensatz needed only for exact `Aut`-naming (C#/meta). |
| **`JointVarietyDeterminesFamily`** | `ScratchRouteC` (F4-multi `recoveredFamily_colouring_equivariant`) | projective normality of Grassmann/spinor variety (span{Q_k} = deg-2 vanishing ideal) | **F4-multi only** | **◑ ROUTE FOUND (§3.2)** — same vanishing-space route as above (multi-dim `W`). |
| **`ConePreservingCollineationIsSemiSimilitude`** | `ScratchRouteC` §F2 (`…_semilinear`) | fundamental theorem of projective geometry (collineations are semilinear) + quadric uniqueness; Artin, *Geometric Algebra* | **F2 only** (`q=pᵉ, e>1`) | **✗ HARD (§3.3)** — FTPG genuinely deep; not elementarily dischargeable. Vacuous at `q=p` (`σ=id`). Keep cited. |
| **`AffineSchemeTwoClosed`** | `ScratchRecoveredFormTransfer.lean` (`schemeAutGroup_affineScheme_eq_affineG` / `routeC_polySupport`) | rank-3 affine 2-closure: `SchemeAutGroup(affineScheme G₀) ≤ affineG G₀` (no unexpected automorphisms); Skresanov arXiv:2007.14696 / 2202.03746. Converse `≥` is **proved** (`affineG_le_schemeAutGroup`). | Route-C coarse-Aut pinning (the `\|Aut\|` side / meta poly) — **one named premise, all four families** via `G₀ := similitudeGroup Q` / `jointConeStab Qs` / Suzuki cone-stab | **○ CITED** — Skresanov rank-3 2-closure; formalizable, off the near-term path. Same instance as the Skresanov row below, now a concrete named Lean `Prop`. |
...
This compiles (verified in `ScratchRouteC` context, axiom-clean):
