...
>
> **LIVE NEXT TASK — Phase 0 (GATE) ✅ probe extended; read it, then the bridge.** (1) **Phase 0 (empirical, do first) —
> `Phase0_BranchProfile` now reports `B`, `L`, `branchFactors`, `nodesByDepth`** (`DescentStats` extended 2026-06-30). Run the
> `q`-sweep (fixed d=4) + `d`-sweep (fixed q=2, d=4,6,8) and read off: does `B` stay `≤ poly(q)` (first signal: q=3 is
> single-path `B=1`, q=5 branches `leaves=6` — `q`-dependent), does `L` track `Θ(d)`, does `leaves` stay poly? Gates the whole
> target. (2) **Phase 1 (Lean bridge):** generalize `ScratchNodeCountBridge` from `SinglePathDisposition` (`B=1`) to
> **`BoundedBranchingDisposition`** (`bᵢ ≤ B`) ⟹ `leaves ≤ Bᴸ` ⟹ poly (carry `L` as an explicit parameter). Then Phase 2
...
**Empirical anchor (Phase 0 — metrics now INSTRUMENTED, `Phase0_BranchProfile`).** `RecoveryReconcileProbe.cs` now reports
`B = MaxBranchFactor`, `L = MaxBranchPathDepth`, and the per-node `branchFactors`/`nodesByDepth` (added to `DescentStats`,
2026-06-30). First readings (default mode): **`VO⁻₄(3)` n=81 is a clean SINGLE PATH** — `B=1, L=0, branchFactors=[]`,
`nodesByDepth=[1,1,1,1,1,1]`, `pruned=120`, `STARVED=0`, full `|Aut|=233280`; whereas the prior `VO⁻₄(5)` reading
**branches** (`leaves=6`, `branchingNodes=4`). **So branching is `q`-DEPENDENT** (single-path at q=3, forks at q=5) — exactly
the `q`-axis the §4 caveat flags, and the first concrete sign that `bᵢ`'s scaling is the crux. **Still to pin (the GATE):**
`B`-vs-`q` and `(leaves, L)`-vs-`d` (`VO⁻₆(3)`, `VO⁻₈(2)`), i.e. whether `bᵢ ≤ poly(q)` and `L = O(d)` hold uniformly.
`STARVED=0` confirms D4/completeness, **not** the leaf-count bound. *(Note: `VO⁻₄(5)` n=625 in default mode is itself slow
— minutes — a per-node-cost signal worth recording alongside the leaf count.)*
...
**Empirical validation (`RecoveryReconcileProbe.cs`, the REAL canonizer):**
...
- **Default mode (T0-relevant):** `q`-DEPENDENT branching — `VO⁻₄(3)` is single-path (`B=1, L=0`), `VO⁻₄(5)` *branches*
  (`branchingNodes=4`, `leaves=6`). So `bᵢ > 1` occurs and grows with `q`; T0 ≠ single path in general.
- **Deferral mode:** `Phase2=0`, `leaves=1` (incl. `VO⁻₆(3)` d=6, full `|Aut|=38093690880`) — the T1/T2 corner.
- **What Phase 0 must still pin (now instrumented — run `Phase0_BranchProfile`):** `B`-vs-`q` (does `bᵢ` stay `≤ poly(q)`?)
  **and** `(leaves, L)`-vs-`d` (does `L` track `Θ(d)` and `leaves` stay poly?). `STARVED=0` confirms D4/completeness, **not**
  the leaf-count bound. The `Phase0_BranchProfile` theory runs a `q`-sweep (fixed d=4) + a `d`-sweep (fixed q=2, d=4,6,8).
...
**Phase 0 — GATE (empirical; do first). ✅ PROBE EXTENDED (2026-06-30).** `RecoveryReconcileProbe.cs` now reports, per run:
`B = MaxBranchFactor`, `L = MaxBranchPathDepth` (exact per-path branching depth, bottom-up — *not* aggregate
`branchingNodes`), the per-node `BranchFactors`/`BranchDepths`, and `NodesByDepth` (all added to `DescentStats`). The
`Phase0_BranchProfile` theory runs a **`q`-sweep** (fixed d=4: q=2,3,5) and a **`d`-sweep** (fixed q=2: d=4,6,8 ⟹ n=16,64,256),
in default (branch-but-resolve) mode. Outcomes to read off:
- **Pin `B`-vs-`q`** (does `bᵢ` stay `≤ poly(q)`, given the q=3→q=5 single-path→branch jump already seen?). This *is* the D2
  predicate to formalize.
- **Pin `L`-vs-`d` and `leaves`-vs-`d`** (does `L` track `Θ(d)`, does `leaves` stay poly?). This is the D1 obligation —
  measured here for the first time, since `≤ n` does not give it.
- **Gate:** if `leaves` (or `L`, or `B`) grows super-poly at `d=6,8` / large `q`, T0 fails on this family — learn it cheaply,
  before any Lean. *(d=6 `VO⁻₆(3)` already canonizes with `STARVED=0`, full `|Aut|` — completeness holds; the leaf-count /
  `bᵢ` / `L` scaling is what was unmeasured and is now instrumented.)*
