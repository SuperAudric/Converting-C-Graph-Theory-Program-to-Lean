# ARCHIVE — formsgraph-WL-dim plan: build history & superseded routes (frozen 2026-06-24)

> **Frozen provenance.** This is the pre-2026-06-24 body of `docs/chain-descent-formsgraph-wldim-plan.md` — the full STATUS
> saga, the superseded `QProfileSeparatesAtBase` / M0–M5 route (§8–§9), the kernel-route HANDOFF and the Lemma A/B build
> records + all spike logs (§10). It is kept for provenance and to avoid re-walking dead ground. **The live doc supersedes
> it**; consult this only for *how* a landed piece was built or *why* a route was abandoned. The `VO⁻₄(3)` instance is
> sealed; the live doc keeps the compact module map + reusable architecture (its §1) and the forward roadmap (its §11).
