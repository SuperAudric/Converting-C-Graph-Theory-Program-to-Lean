# Layered Tiebreak with Supplant — v1 (superseded)

> **Superseded by [`supplant-strategy.md`](./supplant-strategy.md).**
> This document describes the predecessor design — the pair-event scheme with
> atom classification, forced completion, and multi-pass structural sort. It
> is preserved here because the current design (v2) borrows several primitives
> from it (rank tuples, role lex order, the cascade decode) and the v2
> motivation references "what was removed" relative to this draft.
>
> Differences at a glance — v1 → v2:
>
> - **Atom classification** (within-class / cross-class / cross-component) — *removed*. v2 does not classify events structurally; layer ordering is content-determined.
> - **Forced completion** (run all candidate pairs to make atom multisets stable across scrambles) — *removed*. v2 stops at first-singletons and relies on iso-invariant pair pick + post-hoc supplant for cross-scramble agreement.
> - **Multi-pass structural sort** — *removed*. v2 has no per-pass layer reordering; layer order is the order pairs were picked.
> - **Per-event supplant test** — *moved*. v2 decides supplant post-hoc, after the full event loop, asking "does removing this layer collapse any pair?" instead of "did the cascade structurally distinguish A and B?". The v1-style local test was unsound (see v2 §"Bug fix from v1").
> - **Flip canonicalization** — v1 specified per-layer Z/2 lex-min as part of the multi-pass sort; v2 documents the per-layer swap as unsound and leaves correct flip canonicalization to future work.
>
> Read the v2 document for the current algorithm. Read this one only for
> historical context or to mine specific framings (the §9 recursive-singletons
> sketch, the §11 open-questions list).
