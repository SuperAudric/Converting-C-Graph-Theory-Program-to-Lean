...
def builtCost : Nat :=
  Select.descentCostS (Refine.encodeFreeFast (n := 5))
    (Select.selNodeLazyC (RecordKey.recordKey (n := 5))
      (RecordDeepenCell.recordSupplyDeepenC (n := 5))) c5

def sharedCost : Nat :=
  Select.descentCostS (Refine.encodeFreeFast (n := 5))
    (Probe.selNodeLazySK (RecordKey.recordKey (n := 5)) (RecordCost.recordSupplyFast (n := 5))
      (fun c => Deepen.deepenCellSupply c)) c5

#eval show IO Unit from do
  let t0 ← IO.monoMsNow
  IO.println s!"built  = {builtCost}"
  let t1 ← IO.monoMsNow
  IO.println s!"  built  ms = {t1 - t0}"
  let t2 ← IO.monoMsNow
  IO.println s!"shared = {sharedCost}"
  let t3 ← IO.monoMsNow
